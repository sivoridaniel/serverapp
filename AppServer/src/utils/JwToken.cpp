...
string JwToken::generarToken(string email) throw (TokenException)
{
	...
	evaluateOperation(jwt_set_alg(jwt, JWT_ALG_HS256, key256, sizeof(key256)), jwt, (char *) "", MSG_ERROR_HEADER(), logger); //Header: typ: jwt, alg: HS256
	LOG4CPLUS_DEBUG(logger, LOG4CPLUS_TEXT("EMAIL PARA EL PAYLOAD: "<<email<<" "<<jwt));
	evaluateOperation(jwt_add_grant(jwt, "email", email.c_str()), jwt, (char *) "", MSG_ERROR_PAYLOAD(), logger); //Payload -> "username":"xxxx"
	LOG4CPLUS_DEBUG(logger, LOG4CPLUS_TEXT("TIMESTAMP: "<<stimestamp<<" "<<jwt));
	evaluateOperation(jwt_add_grant(jwt, "timestamp", stimestamp.c_str()), jwt, (char *) "", MSG_ERROR_PAYLOAD(), logger); //Payload -> "timestamp":"long int"
	LOG4CPLUS_DEBUG(logger, LOG4CPLUS_TEXT("EXPIRE: "<<seconds_expire<<" "<<jwt));
	evaluateOperation(jwt_add_grant(jwt, "expire", seconds_expire.c_str()), jwt, (char *) "", MSG_ERROR_PAYLOAD(), logger); //Payload -> "expire": "cantidad de segundos en que expira el token"
	LOG4CPLUS_DEBUG(logger, LOG4CPLUS_TEXT("TOKEN GENERADO " <<jwt));
	out = jwt_encode_str(jwt);

	evaluateOperation(0, jwt, out, MSG_ERROR_TOKEN(), logger);

	if (out != NULL) ret_token = out;
...
bool JwToken::isTokenValid(string token) throw (TokenException)
{
	...
	evaluateOperation(jwt_decode(&jwt, token.c_str(), key256, sizeof(key256)), jwt, (char *) "", MSG_ERROR_DECODE_TOKEN(), logger);
...
string JwToken::getEmail(string token) throw (TokenException)
{
	...
	evaluateOperation(jwt_decode(&jwt, token.c_str(), key256, sizeof(key256)), jwt, (char *) "", MSG_ERROR_DECODE_EMAIL(), logger);
