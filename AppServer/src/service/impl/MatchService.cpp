...
bool MatchService::addToYesList(string idUser, string idUserAccepted)
{
	...
	try
	{
		...
		try
		{
			...
			delete matchUser;
			delete matchUserAccepted;
			LOG4CPLUS_ERROR(logger, LOG4CPLUS_TEXT("Los usuarios ya estan matcheados"));
			throw EntityExistsException();
	...
	{
		LOG4CPLUS_ERROR(logger, LOG4CPLUS_TEXT("No se encontro el usuario en la base"));
		throw e;
...
list<UserProfile*> MatchService::getChats(string idUser)
{
	...
	LOG4CPLUS_INFO(logger, LOG4CPLUS_TEXT("Obteniendo la lista de chats del usuario "<<idUser ));
...
void MatchService::confirmUser(string idUser, string idUserConfirmed)
{
	...
	Match* matchUser = (Match*) matchDao->get(idUser);
	Match* matchUserConfirmed = (Match*) matchDao->get(idUserConfirmed);

	if (matchUser->isAccepted(idUserConfirmed))
	{
		delete matchUser;
		delete matchUserConfirmed;
	...
	if (matchUser->isMatched(idUserConfirmed) && matchUserConfirmed->isMatched(idUser))
	{
		...
		matchUserConfirmed->removeFromNewMatches(idUser);
		matchUserConfirmed->addChat(idUser);
		matchDao->put(idUser, matchUser);
		matchDao->put(idUserConfirmed, matchUserConfirmed);
		try
		{
			chatService->createChat(idUser, idUserConfirmed);
			LOG4CPLUS_INFO(logger, LOG4CPLUS_TEXT("Se crea el chat entre el usuario " << idUserConfirmed << " y el usuario " <<idUser));
		} catch (EntityExistsException& e)
		{
			LOG4CPLUS_WARN(logger, LOG4CPLUS_TEXT("Ya existe el chat entre el usuario " << idUserConfirmed << " y el usuario " <<idUser));
		}
		delete matchUser;
		delete matchUserConfirmed;
	}
	else
	{
		delete matchUser;
		delete matchUserConfirmed;
		LOG4CPLUS_ERROR(logger, LOG4CPLUS_TEXT("Los usuarios no estan matcheados"));
		throw IllegalStateException();
	}
