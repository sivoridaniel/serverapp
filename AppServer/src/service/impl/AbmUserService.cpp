...
string AbmUserService::createNewUser(UserProfile* userProfile)
{
	...
	{
		LOG4CPLUS_ERROR(logger, LOG4CPLUS_TEXT("El usuario "<<userProfile->getName()<<" con email " <<userProfile->getEmail()<<" no se pudo crear."));
		delete match;
		throw e;
	} catch (EntityExistsException& e)
	{
		LOG4CPLUS_ERROR(logger, LOG4CPLUS_TEXT("El usuario "<<userProfile->getName()<<" con email " <<userProfile->getEmail()<<" no se puede crear porque ya existe."));
		delete match;
		throw e;
	} catch (RemoteException& e)
	{
		LOG4CPLUS_ERROR(logger, LOG4CPLUS_TEXT("El usuario "<<userProfile->getName()<<" con email " <<userProfile->getEmail()<<" no se puede crear porque falló el post."));
		delete match;
		throw e;
	} catch (exception& e)
	{
		LOG4CPLUS_ERROR(logger, LOG4CPLUS_TEXT("El usuario "<<userProfile->getName()<<" con email " <<userProfile->getEmail()<<" no se puede crear porque fallo la escritura a la db."));
		delete match;
		throw e;
	}
...
void AbmUserService::updateToken(UserProfile* userProfile)
{
	...
	{
		LOG4CPLUS_ERROR(logger, LOG4CPLUS_TEXT("El usuario "<<userProfile->getName()<<" con email " <<userProfile->getEmail()<<" no pudo actualizar su token."));
		throw m;
	} catch (EntityNotFoundException& n)
	{
		LOG4CPLUS_ERROR(logger, LOG4CPLUS_TEXT("El usuario "<<userProfile->getName()<<" con email " <<userProfile->getEmail()<<" no se encuentra."));
		throw n;
	} catch (exception& e)
	{
...
void AbmUserService::modifyUser(UserProfile* userProfile)
{
	...
	{
		LOG4CPLUS_ERROR(logger, LOG4CPLUS_TEXT("El usuario "<<userProfile->getName()<<" con id " <<userProfile->getId()<<" no se pudo actualizar."));
		throw m;
	} catch (EntityNotFoundException& e)
	{
		LOG4CPLUS_ERROR(logger, LOG4CPLUS_TEXT("El usuario "<<userProfile->getName()<<" con email " <<userProfile->getEmail()<<" no se encuentra."));
