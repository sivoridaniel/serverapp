...
string AuthenticationController::connect(struct mg_connection *nc, struct http_message *hm, bool test)
{
...
string AuthenticationController::event_handler_login_user(struct mg_connection *nc, struct http_message *hm)
{
	...
	try
	{
		...
		userProfileConsultado = this->authenticationService->getUserLogin(userProfileBuscado->getEmail(), userProfileBuscado->getPassword());

		if (userProfileConsultado != NULL)
		{ //Si se pudo loguear, genera el token y modifica el usuario para guardarlo
