...
string MatchController::connect(struct mg_connection *nc, struct http_message *hm, bool test)
{
...
string MatchController::event_handler_submit_yes(struct mg_connection *nc, struct http_message *hm, string token)
{
	...
	try
	{
		...
		/* Call match service */
		LOG4CPLUS_DEBUG(logger, LOG4CPLUS_TEXT("Se intenta agregar el usuario " << idTo << " a la lista de aceptados de " << idFrom));
		...
		try
		{
			...
			if (matched)
			{
				LOG4CPLUS_INFO(logger, "se produzco un match entre "<<idFrom<<" y "<<idTo);
				json = this->getGenericJson("true", "matched");
			}
			else
			{
				LOG4CPLUS_INFO(logger, "se agrega el usuario "<<idTo<<" a la lista de aceptados de "<<idFrom);
...
string MatchController::event_handler_submit_no(struct mg_connection *nc, struct http_message *hm, string token)
{
	Logger logger = Logger::getInstance(LOG4CPLUS_TEXT("MatchController"));
	...
	try
	{
		...
		/* Call match service */
		LOG4CPLUS_DEBUG(logger, LOG4CPLUS_TEXT("Se intenta agregar el usuario " << idTo << " a la lista de rechazados de " << idFrom));

		try
		{
			matchService->addToNoList(idFrom, idTo);
			LOG4CPLUS_INFO(logger, "se agrega el usuario "<<idTo<<" a la lista de rechazados de "<<idFrom);
...
string MatchController::event_handler_confirm_match(struct mg_connection *nc, struct http_message *hm, string token)
{
	...
	try
	{
		...
		/* Call match service */
		LOG4CPLUS_DEBUG(logger, LOG4CPLUS_TEXT("Se intenta agregar el usuario " << idTo << " a la lista de confirmados de " << idFrom));

		try
		{
			matchService->confirmUser(idFrom, idTo);
			LOG4CPLUS_INFO(logger, "se agrega el usuario "<<idTo<<" a la lista de confirmados de "<<idFrom);
...
string MatchController::event_handler_new_matches(struct mg_connection *nc, struct http_message *hm, string token)
{
	Logger logger = Logger::getInstance(LOG4CPLUS_TEXT("MatchController"));
	...
	else
	{
		...
		LOG4CPLUS_DEBUG(logger, LOG4CPLUS_TEXT("Se obtienen los nuevos matches del usuario " << id));
...
string MatchController::event_handler_chats(struct mg_connection *nc, struct http_message *hm, string token)
{
	...
	else
	{
		...
		LOG4CPLUS_DEBUG(logger, LOG4CPLUS_TEXT("Se obtienen los chats del usuario " << id));
